...
//	return warpedRegion;
 ...
 //AlprFullDetails AlprImpl::split3impl(cv::Mat grayImg, std::queue<PlateRegion> plateQueue,
  ...
  SplitReturn AlprImpl::recognize( unsigned char* pixelData, int bytesPerPixel, int imgWidth, int imgHeight, std::vector<AlprRegionOfInterest> ROI)
  {
	timespec startTime;
    getTimeMonotonic(&startTime);
    ...
    std::vector<cv::Rect> regionsOfInterest = this->convertRects(ROI);
    AlprFullDetails response;
